...
完整的 22 条模式 + 改写示例，供 [SKILL.md](SKILL.md) 阶段一执行时查阅。识别文本中的具体痕迹时打开本文件；不需要全部背下来，按需对照。
...
**问题：** 英文版本的这个模式是在句尾追加 -ing 分词短语（如 "highlighting...", "ensuring..."）制造虚假深度；中文没有 -ing，对应的表现是用"……，象征着……，反映了……"这类并列状语层层叠加，看似在分析，实际没有给出任何新信息。
...
> 注意：这条与阶段二的"表格/列表禁用内联语法"规则呼应——粗体过度使用在正文段落里是内容问题，在 Markdown 表格/列表里则是飞书渲染问题，两层都要查。
...
> **仅英文适用，中文文档可跳过：** 英文 AI 写作还有两个常见痕迹在中文里没有对应——
> 标题逐词首字母大写（Title Case，中文标题没有大小写之分）；
> 用弯引号 “ ” 而不是直引号 " "（中文应使用中文引号「」或全角引号""）。
> 如果中文文档里混入了这两种英文排版习惯，按对应规则改掉即可，不必单独当作两条规则记。
